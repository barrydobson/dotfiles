...
#parser.add_argument('token', help='token from your MFA device', default="")
...
print('Touch YubiKey')
ykmantoken = subprocess.run(['ykman', 'oath', 'code', '-s', 'awstote'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
tkn=ykmantoken.stdout.decode('utf-8')[0:6]

usernameresult = subprocess.run(['aws', 'iam', 'get-user', '--profile', 'default', '--output', 'text', '--query', 'User.UserName'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
username = usernameresult.stdout.decode('utf-8').strip('\n')
...
result = subprocess.run(['aws', 'sts', 'assume-role', '--role-arn', 'arn:aws:iam::' + assume_account + ':role/' + assume_role, '--role-session-name', username, '--profile', 'default', '--serial-number', args.arn, '--token-code', tkn], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
